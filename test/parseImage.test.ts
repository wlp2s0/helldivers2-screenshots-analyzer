...
test("[parseImage] should return expected success rate", async () => {
	const targetColours = [
		{
			colour: { r: 255, g: 135, b: 36, tolerance: 32 },
			label: "primary",
		},
		{
	...
	for (const file of files) {
		...
		const primaryResult = await parseImage({
			filename,
			sourcePath,
			targetColor: primaryColour.colour,
			label: primaryColour.label,
			debug: true,
			baseDebugPath: "./debug",
		});
		strictEqual(primaryResult.success, Number.parseInt(expectedPrimarySuccess), `Primary missions failed: ${sourcePath}`);

		const secondaryResult = await parseImage({
			filename,
			sourcePath,
			targetColor: secondaryColour.colour,
